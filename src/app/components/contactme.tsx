...
export default function Contactme() {
  ...
  const onSubmit: SubmitHandler<IFormInput> = (data) =>{
    ...
    emailjs
    .sendForm('service_kdi8p2v', 'template_6fzl7ge', form.current, {
    ...
    .then(
      ...
      (error) => {
        toast.error('A ocurrido un error al enviar el mensajer!', {
        ...
        <form ref={form} onSubmit={handleSubmit(onSubmit)} className=' w-full flex flex-col gap-6 items-center justify-center p-10 shadow-lg'>
            ...
            <div className=' w-full grid grid-cols-1'>
                  ...
                  {/* <div>
                    ...
                    className='w-full h-16 p-4 shadow-md'
                    type='text'
                    placeholder='tu numero movil'
                    {...register("phone", { required:true })}
                    aria-invalid={errors.message ? "true" : "false"}
                    />
                    {errors.phone?.type === "required" && (
                    <p className='text-red-600 animate-jump' role="alert">*El campo telefono movil es requerido</p>
                    )}
                  </div> */}
            </div>
            ...
            <button className=' pl-9 pr-9 pt-3 pb-3 bg-main-color text-[1.7rem] text-white rounded-lg'>Enviar mensaje</button>
        </form>
    </section>
    </>
  )
